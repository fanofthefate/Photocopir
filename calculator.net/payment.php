...
$main_mail = mail("dedtorpedodet@gmail.com", $tema, $pismo, "From: site@photocopir.ru" . "\r\n" . "Reply-To: ". $mail ."\r\n". $headers,"-f dedtorpedodet@gmail.com");
// $send_mail = mail("crion31@gmail.com", "$tema", "$pismo", "From: site@photocopir.ru" . "\r\n" . "Reply-To: ". $mail ."\r\n". $headers,"-f site@photocopir.ru");

$admin_mail = mail($mail, $sbj,utf("Информация о заказе:"). '<br>' .  $string . '<br>' . utf($info) . '<br>' . utf('Прикрепленные файлы:') . '<br>' . $filesString, 'From: site@photocopir.ru' . "\r\n" .
    'Reply-To: '. $to . "\r\n" .$headers);
//$send_mail = mail($to, $sbj_client, utf('Номер вышего заказа: ') . $PRODUCT_ID . '<br>' . '<br>' . $string . '<br>' . utf($info) , $headers);
$client_mail = mail($to, $sbj_client, utf("Здравствуйте. От вас был получен онлайн-заказ в студии Фотокопир. Номер вашего заказа "). $PRODUCT_ID.
utf(". Tак же информация о заказе доступна из вашего личного кабинета: http://photocopir.ru/personal/orders/. ").'<br>'.
    ...
    'Reply-To: '. $mail . "\r\n" .$headers);
$main_mail = mail('dedtorpedodet@gmail.com', $sbj, $pismo . '<br>' . $filesString, 'From: site@photocopir.ru' . "\r\n" .
...
v>
    <button id="confirm_but" onclick="pay();" style="width: 130px;
    font-size: 12pt;
    font-weight: bold;margin-top: 30px;">РћРїР»Р°С‚РёС‚СЊ
    </button>
</section>
<script>
    // function pay() {
